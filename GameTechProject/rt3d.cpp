...
void setMaterial(const GLuint program, const materialStruct material) {
	...
	uniformIndex = glGetUniformLocation(program, "material.diffuse_weight");
	glUniform1f(uniformIndex, material.diffuse_weight);
	uniformIndex = glGetUniformLocation(program, "material.specular_weight");
	glUniform1f(uniformIndex, material.specular_weight);
}
