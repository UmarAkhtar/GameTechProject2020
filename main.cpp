...
void init(void) {
	...
	gouraud = rt3d::initShaders("gouraud.vert", "gouraud.frag");
	rt3d::setLight(gouraud, light0);
	rt3d::setMaterial(gouraud, material0);
...
const char *cubeTexFiles[6] = {
	...
	shaderSwitch = toonShaderProgram;
	shaderSwitch2 = shaderProgram;
...
void groundDraw()
{
	...
	//glBindTexture(GL_TEXTURE_2D, textures[0]);
	//mvStack.push(mvStack.top());
	//mvStack.top() = glm::translate(mvStack.top(), glm::vec3(-10.0f, 5.0f, -10.0f));
	//mvStack.top() = glm::scale(mvStack.top(), glm::vec3(20.0f, 0.1f, 20.0f));
	//rt3d::setUniformMatrix4fv(shaderProgram, "modelview", glm::value_ptr(mvStack.top()));
	//rt3d::setMaterial(shaderProgram, material0);
	//rt3d::drawIndexedMesh(meshObjects[0], meshIndexCount, GL_TRIANGLES);
...
void cubeDraw(glm::vec4 &tmp, glm::mat4 &projection)
{
	...
	glBindTexture(GL_TEXTURE_2D, textures[2]);
	mvStack.push(mvStack.top());
	mvStack.top() = glm::translate(mvStack.top(), glm::vec3(-27.0f, 1.0f, -3.0f));
